...
def create_callback(): # Esto es una función que devuelve una funcion... Es la "función constructora".
	...
	pid.print_config()
	def callback(indata, outdata, frames, time_, status): # Prototipo del callback de sounddevice.
		# ~ print('---')
		...
		print(str(time.time() - t_start) + '\t' + str(DESIRED_SNR) + '\t' + str(SNR) + '\t' + str(DESIRED_SNR - SNR) + '\t' + str(amplitude))
	return callback
